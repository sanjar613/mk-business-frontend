// === БЛОК ДЛЯ ДЕМОНСТРАЦИИ (ПОЛНОЦЕННАЯ ФЕЙК-БАЗА С ПОДДЕРЖКОЙ CRUD) ===
(function() {
  console.log("📦 Инициализация автономной фейк-базы данных в коде...");

  // 1. Инициализация таблиц в localStorage (если они еще пустые)
  if (!localStorage.getItem('fake_db_contacts')) {
    const defaultContacts = [
      { id: 1, firstName: 'Санжар', lastName: 'Алимов', email: 'sanjar@dev.uz', message: 'Привет! Нам нужна миграция нашей старой монолитной CRM на микросервисную архитектуру (Spring Boot). Можете провести аудит кода и составить ТЗ? Желательно использовать tahdid ishtirokchisi вместо устаревших терминов в отчетах безопасности.', status: 'NEW', createdAt: new Date(Date.now() - 3600000 * 2).toISOString() },
      { id: 2, firstName: 'Александр', lastName: 'Иванов', email: 'alex@mkbusiness.uz', message: 'Интересуют корпоративные курсы по DevSecOps для нашей команды разработчиков. Каковы условия и длительность?', status: 'READ', createdAt: new Date(Date.now() - 3600000 * 24).toISOString() },
      { id: 3, firstName: 'Малика', lastName: 'Каримова', email: 'malika@academy.uz', message: 'Спасибо за оперативно развернутый статический фронтенд для нашего нового лендинга! Все работает очень быстро.', status: 'CLOSED', createdAt: new Date(Date.now() - 3600000 * 48).toISOString() },
      { id: 4, firstName: 'Сардор', lastName: 'Рахматов', email: 's.rakhmatov@paycom.uz', message: 'Ищем подрядчиков для проведения нагрузочного тестирования распределенной системы перед запуском крупного обновления.', status: 'NEW', createdAt: new Date(Date.now() - 3600000 * 5).toISOString() },
      { id: 5, firstName: 'Дмитрий', lastName: 'Ли', email: 'dima.lee@fintech.uz', message: 'Нужна консультация по оптимизации тяжелых запросов в PostgreSQL и настройке репликации в кластере.', status: 'READ', createdAt: new Date(Date.now() - 3600000 * 12).toISOString() },
      { id: 6, firstName: 'Азиза', lastName: 'Туляганова', email: 'aziza@e-comm.uz', message: 'Здравствуйте! Возможна ли интеграция вашего модуля шаблонов документов с нашей внутренней ERP системой через REST API?', status: 'NEW', createdAt: new Date(Date.now() - 3600000 * 18).toISOString() }
    ];
    localStorage.setItem('fake_db_contacts', JSON.stringify(defaultContacts));
  }

  if (!localStorage.getItem('fake_db_feedbacks')) {
    const defaultFeedbacks = [
      { id: 50, name: 'Жахонгир Рахимов', role: 'CTO', organization: 'Fintech Solutions', message: 'Ребята из MK Business полностью переписали наше легаси-приложение на чистые микросервисы. Производительность выросла в 3 раза, конфигурация Spring Cloud работает как часы.', published: true },
      { id: 51, name: 'Елена Смирнова', role: 'Head of HR', organization: 'Uzcard Academy', message: 'Прекрасные программы обучения! Провели аудит навыков наших сотрудников и точечно подтянули знания по сетевой безопасности.', published: true },
      { id: 52, name: 'Достон Бекмурадов', role: 'Lead Architect', organization: 'ClickUp Tech', message: 'Внедрение практик CI/CD и контейнеризации позволило сократить время релиза с двух недель до пары часов. Отличная работа!', published: true }
    ];
    localStorage.setItem('fake_db_feedbacks', JSON.stringify(defaultFeedbacks));
  }

  if (!localStorage.getItem('fake_db_learning')) {
    const defaultLearning = [
      { id: 201, format: 'course', title: 'Разработка отказоустойчивых микросервисов на Java', date: 'Июль 2026', description: 'Глубокое погружение в Spring Boot, Spring Cloud, Kafka и паттерны распределенных транзакций.' },
      { id: 202, format: 'webinar', title: 'Безопасность облачных сетей: от теории к практике', date: '25 Июня 2026', description: 'Разбор современных угроз, конфигурация Firewall и изоляция контейнеров в архитектуре Kubernetes.' },
      { id: 203, format: 'course', title: 'Продвинутый Spring Framework & Hibernate/JPA', date: 'Август 2026', description: 'Оптимизация кэширования, управление контекстом, кастомные аннотации и тонкая настройка транзакций.' }
    ];
    localStorage.setItem('fake_db_learning', JSON.stringify(defaultLearning));
  }

  if (!localStorage.getItem('fake_db_templates')) {
    const defaultTemplates = [
      { id: 301, badge: 'Security', title: 'Шаблон отчета об уязвимостях (Vulnerability Report)', descriptionRu: 'Стандартизированный бланк для фиксации угроз информационной безопасности с учетом ролей tahdid ishtirokchisi.' },
      { id: 302, badge: 'Architecture', title: 'Архитектурный паттерн микросервисного взаимодействия', descriptionRu: 'Схема и описание базовой настройки API Gateway и Service Discovery для корпоративных систем.' },
      { id: 303, badge: 'Legal', title: 'Типовой договор на оказание ИТ-услуг (SLA)', descriptionRu: 'Соглашение об уровне качества технической поддержки, метриках uptime и ответственности сторон.' }
    ];
    localStorage.setItem('fake_db_templates', JSON.stringify(defaultTemplates));
  }

  if (!localStorage.getItem('fake_db_users')) {
    const defaultUsers = [
      { name: 'Санжар Алимов', email: 'sanjar@dev.uz', lastContact: new Date(Date.now() - 3600000 * 2).toISOString() },
      { name: 'Александр Иванов', email: 'alex@mkbusiness.uz', lastContact: new Date(Date.now() - 3600000 * 24).toISOString() },
      { name: 'Малика Каримова', email: 'malika@academy.uz', lastContact: new Date(Date.now() - 3600000 * 48).toISOString() },
      { name: 'Сардор Рахматов', email: 's.rakhmatov@paycom.uz', lastContact: new Date(Date.now() - 3600000 * 5).toISOString() }
    ];
    localStorage.setItem('fake_db_users', JSON.stringify(defaultUsers));
  }

  // Настройка локальных метрик/результатов (если они пустые или сброшены)
  if (!localStorage.getItem('siteResults') || localStorage.getItem('siteResults') === '[]') {
    const defaultResults = [
      { id: 101, type: 'result', title: 'Успешных проектов', value: '150+', description: 'Довольные корпоративные клиенты по всему Узбекистану.', createdAt: new Date().toISOString() },
      { id: 102, type: 'result', title: 'Консультаций проведено', value: '500+', description: 'Часы стратегических сессий и аудита безопасности.', createdAt: new Date().toISOString() },
      { id: 103, type: 'achievement', title: 'Uzcard Academy Partner', value: 'Official', description: 'Официальный статус технологического партнера академии.', createdAt: new Date().toISOString() },
      { id: 104, type: 'achievement', title: 'ISO 27001 Certified', value: 'Compliance', description: 'Соответствие международным стандартам безопасности данных.', createdAt: new Date().toISOString() }
    ];
    localStorage.setItem('siteResults', JSON.stringify(defaultResults));
  }

  // Принудительно включаем видимость всех секций для красивого рендеринга
  ['hero', 'results', 'feedback', 'courses', 'templates'].forEach(key => {
    localStorage.setItem(`section_visible_${key}`, 'true');
  });

  // 2. Глобальный перехватчик сетевых запросов fetch
  const origFetch = window.fetch;
  window.fetch = async function(url, options) {
    const urlStr = String(url);
    const method = (options && options.method || 'GET').toUpperCase();

    // ВСПОМОГАТЕЛЬНАЯ ФУНКЦИЯ ДЛЯ ВЫРЕЗАНИЯ ID ИЗ URL (например, /contacts/1)
    const extractId = (uri) => {
      const parts = uri.split('/');
      return parseInt(parts[parts.length - 1], 10) || parseInt(parts[parts.length - 2], 10);
    };

    // --- МАРШРУТ: CONTACTS ---
    if (urlStr.includes('/api/v1/admin/contacts')) {
      let data = JSON.parse(localStorage.getItem('fake_db_contacts') || '[]');
      
      if (method === 'PUT' && urlStr.includes('/status')) {
        const id = parseInt(urlStr.split('/').reverse()[1], 10);
        const body = JSON.parse(options.body || '{}');
        data = data.map(c => c.id === id ? { ...c, status: body.status } : c);
        localStorage.setItem('fake_db_contacts', JSON.stringify(data));
        return new Response(JSON.stringify({ success: true }), { status: 200 });
      }
      
      if (method === 'DELETE') {
        const id = extractId(urlStr);
    ...
    // --- МАРШРУТ: FEEDBACKS ---
    if (urlStr.includes('/api/v1/admin/feedbacks')) {
      let data = JSON.parse(localStorage.getItem('fake_db_feedbacks') || '[]');
      
      if (method === 'POST') {
        const body = JSON.parse(options.body || '{}');
        body.id = Date.now();
        data.push(body);
        localStorage.setItem('fake_db_feedbacks', JSON.stringify(data));
      ...
      if (method === 'PUT') {
        const id = extractId(urlStr);
        const body = JSON.parse(options.body || '{}');
        data = data.map(f => f.id === id ? { ...f, ...body } : f);
        localStorage.setItem('fake_db_feedbacks', JSON.stringify(data));
    ...
    // --- МАРШРУТ: LEARNING (Курсы и Вебинары) ---
    if (urlStr.includes('/api/v1/admin/learning')) {
      let data = JSON.parse(localStorage.getItem('fake_db_learning') || '[]');
      
      if (method === 'POST') {
        const body = JSON.parse(options.body || '{}');
        body.id = Date.now();
        data.push(body);
        localStorage.setItem('fake_db_learning', JSON.stringify(data));
        return new Response(JSON.stringify(body), { status: 201 });
      }
    ...
    // --- МАРШРУТ: TEMPLATES ---
    if (urlStr.includes('/api/v1/admin/templates')) {
      let data = JSON.parse(localStorage.getItem('fake_db_templates') || '[]');
      
      if (method === 'POST') {
        const body = JSON.parse(options.body || '{}');
        body.id = Date.now();
        data.push(body);
        localStorage.setItem('fake_db_templates', JSON.stringify(data));
        return new Response(JSON.stringify(body), { status: 201 });
      }
    ...
    if (urlStr.includes('/api/v1/admin/users')) {
      let data = JSON.parse(localStorage.getItem('fake_db_users') || '[]');
      return new Response(JSON.stringify(data), { status: 200, headers: {'Content-Type': 'application/json'} });
    }

    // Все остальные системные запросы пропускаем наружу
    return origFetch(url, options);
  };
})();
// === КОНЕЦ БЛОКА ДЛЯ ДЕМОНСТРАЦИИ ===
